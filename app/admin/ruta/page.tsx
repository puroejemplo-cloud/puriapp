...
import type { PuntoMapa } from '@/components/MapaVentas'

// Carga dinámica del mapa (Leaflet no funciona en SSR)
const MapaVentas = lazy(() => import('@/components/MapaVentas'))
  ...
  return (
    <div className="max-w-3xl">
        ...
        <div className="ml-auto flex rounded-xl overflow-hidden border border-gray-200">
          <button
            ...
            className={`px-3 py-1.5 text-sm font-medium transition-colors ${
              vista === 'mapa' ? 'bg-sky-500 text-white' : 'bg-white text-gray-600 hover:bg-gray-50'
            }`}
          >
            🗺 Mapa
          </button>
          <button
            ...
            className={`px-3 py-1.5 text-sm font-medium transition-colors ${
              vista === 'lista' ? 'bg-sky-500 text-white' : 'bg-white text-gray-600 hover:bg-gray-50'
            }`}
          >
            📋 Lista
          </button>
      ...
      <div className="flex items-center gap-3 mb-4">
        <button
          onClick={() => setRuta(calcularRutaOptima(inicio ?? { lat: 0, lng: 0 }, pedidos))}
        ...
        {ruta.length > 0 && (
          <button onClick={() => setRuta([])} className="text-sm text-gray-400 hover:text-gray-600">
            Mostrar por fecha
          </button>
        )}
        {!inicio && (
      ...
      {sinUbicacion > 0 && pedidos.length > 0 && (
        <p className="text-xs text-amber-600 mb-2 px-1">
          ⚠️ {sinUbicacion} pedido{sinUbicacion > 1 ? 's' : ''} sin coordenadas — no aparece{sinUbicacion > 1 ? 'n' : ''} en el mapa
        </p>
      )}

      {/* MAPA */}
      {vista === 'mapa' && !cargando && (
        <Suspense fallback={<div className="h-96 bg-gray-50 rounded-2xl flex items-center justify-center text-gray-400 text-sm">Cargando mapa...</div>}>
          <MapaVentas puntos={puntosMapa} centro={inicio ?? undefined} />
        </Suspense>
      )}
